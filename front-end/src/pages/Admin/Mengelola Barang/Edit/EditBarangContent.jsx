...
export default function EditBarangContent () {
  ...
  const [status_barang, setStatusBarang] = useState(data2[0]);
    ...
    const fetchDetail = async () => {
      try {
        ...
        if (response.success) {
          ...
          console.log(detail.status_barang);
          setStatusBarang(data2.find(datas => datas.name === (detail.status_barang || '')) || data2[0]);
        } else {
  ...
  return (
    <div className="p-4 md:p-10 lg:px-14">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col space-y-5">
        ...
        <InputSelect data={data2} label={'Status Barang'} defaultValue={status_barang} classLabel={''} onChange={handleStatusChange} />
