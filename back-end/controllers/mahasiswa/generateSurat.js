...
const generatePinjamRuang = async (req,res) => {
    try {
        ...
        const fileName = `${nama_generate}_${findMhs.nama_mahasiswa}_PeminjamanRuangan.docx`;
...
const generatePinjamBarang = async (req,res) => {
    try {
        const {id_jenis} = req.params
        let i = 1;
        const nim_mahasiswa = req.mahasiswa.nim_mahasiswa
        ...
        const {nama_generate, keperluan_peminjaman_barang, tanggal_peminjaman_barang, id_barang} = req.body
        let total = 0
        const findAllGenerate = await modelGenerateSurat.findAll()
        if (findAllGenerate.length <= 0) {
            total = total + 1
        ...
        const fileName =  `${nama_generate}_${findMhs.nama_mahasiswa}_PeminjamanBarang.docx`;
        const outputPath = path.resolve(__dirname, '../', '../', 'public', 'doc', 'generate', fileName);
        fs.writeFileSync(outputPath, buf)
